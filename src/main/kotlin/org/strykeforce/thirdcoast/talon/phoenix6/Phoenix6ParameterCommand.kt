...
class Phoenix6ParameterCommand(
    ...
    override val menu: String
        get() {
            ...
            return when(param.enum) {
                ROTOR_OFFSET -> formatMenu(config.Feedback.FeedbackRotorOffset)
                SENSOR_TO_MECH_RATIO -> formatMenu(config.Feedback.SensorToMechanismRatio)
                ROTOR_TO_SENSOR_RATIO -> formatMenu(config.Feedback.SensorToMechanismRatio)
                ...
                SUPPLY_V_TIME_CONST -> formatMenu(config.Voltage.SupplyVoltageTimeConstant)
                ...
                REV_SOFT_THRES -> formatMenu(config.SoftwareLimitSwitch.ForwardSoftLimitThreshold)
    ...
    private fun configDoubleParam(default: Double, configure: (TalonFX, Double) -> Unit) {
        ...
        logger.debug { "set ${talonFxService.active.size} talonFx's ${param.name}: $paramValue" }
    }
