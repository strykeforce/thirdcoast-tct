...
class P6RunClosedLoopTwoArgsCommand(
    parent: Command?,
    key: String,
    toml: TomlTable
    ...
    override fun execute(): Command {
        ...
        while(!done) {
            try {
                ...
                val arg2 = if (setpoints.size > 1) setpoints[1].toDouble() else 0.0
                ...
                if(type == CTRE_ClosedLoopType.Motion_Magic && arg2 > 0.0) {
                    terminal.warn("second parameter is ignored for Motion Magic type closed loop modes")
                }

                logger.info { "Setpoint: $setpoint, Argument 2: $arg2, Duration: $duration" }

                //run talon
                when (device) {
                    "fx" -> {
                        if (bus == "rio") {
                            talonFxService.active.forEach { it.runClosedLoop(setpoint, arg2) }
                        } else if (bus == "canivore") {
                            talonFxFDService.active.forEach { it.runClosedLoop(setpoint, arg2) }
                        } else throw IllegalArgumentException()
                    }

                    "fxs" -> {
                        if (bus == "rio") {
                            talonFxsService.active.forEach { it.runClosedLoop(setpoint, arg2) }
                        } else if (bus == "canivore") {
                            talonFxsFDService.active.forEach { it.runClosedLoop(setpoint, arg2) }
                        } else throw IllegalArgumentException()
                    }
                }
